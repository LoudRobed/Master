/*
 *  MedeaAggregationPerceptronControlArchitecture.cpp
 *  roborobo-online
 *
...
MedeaAggregationPerceptronControlArchitecture::MedeaAggregationPerceptronControlArchitecture( RobotAgentWorldModel *__wm )
{
	_wm = (MedeaAggregationAgentWorldModel*)__wm;
	_iteration = 0;

	//_initialEnergy = 2*MedeaAggregationSharedData::gEvaluationTime*0.1;  // original CEC
	//_initialEnergy = 2*MedeaAggregationSharedData::gEvaluationTime;  // original CEC, adapte avec nv schema
	//_initialEnergy = MedeaAggregationSharedData::gEvaluationTime*0.1 / 2; // half the maximum lifetime.  // used to be: 2*MedeaAggregationSharedData::gEvaluationTime*0.1;
	//_initialEnergy = MedeaAggregationSharedData::gEvaluationTime*2; // MUST BE AT LEAST MedeaAggregationSharedData::gEvaluationTime otw a dead agent will never be revived  // / 2.0; // half the maximum lifetime + nv schema (entier)
	
	_wm->setEnergyLevel(MedeaAggregationSharedData::gEnergyInit);
	//_deltaEnergy = 10.0; // CEC
	_wm->setDeltaEnergy(0.0); // MedeaAggregation
...
MedeaAggregationPerceptronControlArchitecture::~MedeaAggregationPerceptronControlArchitecture()
{
	// nothing to do.
}

void MedeaAggregationPerceptronControlArchitecture::reset()
{
	_parameters.clear();
...
void MedeaAggregationPerceptronControlArchitecture::step()
{
	_iteration++;
...
/*	if ( _wm->_age < 0 ) // problem: _age is nowhere to be incremented
	{
		...
		currentObserver->setKey( ( ranf()*2.-1. ) * MedeaAggregationSharedData::gMaxKeyRange);
		return;
	}
*/
